...
const _conversationprompt = require("./prompts/conversation.prompt");
const _grammarprompt = require("./prompts/grammar.prompt");
...
let AiService = class AiService {
    async generateResponse(userId, conversationId, userMessage) {
        ...
        const systemPrompt = (0, _conversationprompt.conversationPrompt)({
        ...
        const grammarCheck = await this.llm.chat((0, _grammarprompt.grammarPrompt)(), history, userMessage, true);
        let parsedGrammar = {};
        try {
    ...
    async generateFeedback(userId, conversationId) {
        const conversation = await this.prisma.conversation.findUnique({
            where: {
                id: conversationId
            },
...
Provide feedback as a JSON object with keys: grammarMistakes, vocabularyScore, fluencyScore, suggestions, strengths.`;
    ...
    async recordMistakes(userId, grammar) {
